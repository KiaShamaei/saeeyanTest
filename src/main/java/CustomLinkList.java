...
public class CustomLinkList<K, V>  {
    ...
    CustomLinkList(KeyValueNode<K, V> head, int numberOfNodes) {
            this.head = head;
        ...
        V fetchValueInNodeWithKey(K key) throws Exception {
            KeyValueNode<K, V> node = fetchNodeWithKey(key);
            if (node == null)
        ...
        CustomKeyValue<K, V> getPojoOfNodeAtIndex(int index) {
        ...
        Stream<CustomKeyValue<K, V>> getAllPojo() {
            return allNodesStream().map(node -> new CustomKeyValue<>(node.key(), node.value()));
        }
    }
